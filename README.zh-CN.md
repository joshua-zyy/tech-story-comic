...
故事层还给每一页设定了**目标节奏**——我们希望 Codex 产出时达到的效果。它是工艺靶心，不是套模版（示例来自一套用 8 页讲网络流的漫画）：

- **每页三拍**：学习者的“想要/误解”（小格）→ 概念被命名并**用一张大图示一次演完**（最大格，占半页以上，文字最少）→ 一句人话落地（小格）。纯对话页和纯黑板页读起来就是上课。
- **先演错，再命名**：学习者先用身体犯一次错，导师再给出术语。
- **每页一个转折（适合主题时）**：许愿 → 打脸 → 给新工具，这是让约束条件被记住的关键。
- **每页结尾留一个悬而未决的限制**，下一页接住：“我懂了 A，那如果是 B 呢？”
- **问题页配对解决页**；避免连续两页重理论；大约每四页留一页纯奇观页。
- **终章以一个大远景收束** + 一句回归日常的损梗——绝不以公式结尾。
