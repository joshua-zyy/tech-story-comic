...
description: Story-driven technical explainer comics on the proven knowledge-comic engine. Characters hit a concrete dilemma first, concepts arrive one by one as the story needs them, and the ending resolves the problem with what was learned. Use when user asks to create "技术讲解漫画", "技术科普漫画", "知识漫画", "教育漫画", "tutorial comic", or "Logicomix-style comic".
version: 1.117.4
metadata:
  openclaw:
    homepage: https://github.com/joshua-zyy/tech-story-comic
    requires:
      anyBins:
        - bun
        - npx
...
## Story Line Layer ⛔ REQUIRED

This skill is a thin story layer over the proven knowledge-comic engine: every visual, storyboard, and generation mechanism is upstream and unchanged. The layer adds one requirement — the comic must be a **story**, not a topic tour.

Before content analysis (Step 1.2), read [story-analysis.md](references/story-analysis.md) and fix the story line:
...
| 1.2 | Analyze content + fix story line ([story-analysis.md](references/story-analysis.md)) | `analysis.md` |
...
**Story Layer** (this skill's addition):
- [story-analysis.md](references/story-analysis.md) - Story line, concept chain, and ending check ⛔ read before Step 1.2
