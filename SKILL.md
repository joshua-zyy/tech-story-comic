---
name: tech-story-comic
description: Story-driven technical explainer comic creator. Turns ONE technical topic into a multi-page comic where characters face a concrete problem and each concept arrives because the story needs it. Use when the user asks for a "技术讲解漫画", "技术科普漫画", "tech explainer comic", "technical comic", or wants to turn a technical topic into a story-driven multi-page comic with consistent characters.
version: 0.1.0
metadata:
  openclaw:
    requires:
...
Create story-driven technical explainer comics: one topic, one problem-driven storyline, concepts introduced as the story needs them.
...
1. **Built-in Codex `imagegen`** — inspect your available-skills / tool inventory. If a skill named `imagegen` is listed, you are running inside Codex and MUST use it: invoke via the `Skill` tool with `skill: "imagegen"`, passing the saved prompt file's content (plus output path and aspect ratio per Codex `imagegen`'s own args). Codex `imagegen` is the official raster backend in this runtime.
2. **If it is not available**, tell the user plainly — this skill's first release targets Codex's built-in image generation only — and ask how to proceed. Do not silently switch to external APIs, paid services, or wrapper CLIs.

**⛔ Never substitute SVG, HTML, canvas, or other code-based rendering for raster image generation.** Codex `imagegen`'s own description says it should be used "when the output should be a bitmap asset rather than repo-native code or vector." If you cannot resolve a raster backend via the rule above, stop and ask the user — do **not** silently emit SVG, write inline `<svg>` markup, or produce HTML/CSS art as a substitute. This applies even if the article/section seems "diagram-like": the consumer skill calling this rule has already decided that a raster image is what it needs.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.tech-story-comic/EXTEND.md` | Project |
| 2 | `$HOME/.tech-story-comic/EXTEND.md` | User home |
...
**EXTEND.md supports**: watermark, preferred art/tone/layout, custom style definitions, character presets, language preference, generation batch size. Schema: [references/config/preferences-schema.md](references/config/preferences-schema.md).
...
- Maintain character consistency via the character reference sheet, not session IDs
...
EXTEND.md lives at `.tech-story-comic/EXTEND.md` (project) or `~/.tech-story-comic/EXTEND.md` (user). Three ways to change it:

- **Edit directly** — open EXTEND.md and change fields. Full schema: `references/config/preferences-schema.md`.
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure tech-story-comic preferences" / "重新配置"). The next run re-triggers first-time setup.
- **Common one-line edits**:
  - `generation_batch_size: 4` — default number of page images to render concurrently when the runtime supports parallel generation.
